...
(function () {
  ...
  function init() {
    ...
    Store.ensure(NS, { days: {}, brainDump: "", pillars: [], habits: [], habitLog: {}, reviews: {}, journal: {}, thesis: { targetDate: "", milestones: [], tasks: [] } });
    seedIfEmpty();
    dedupeHabitsPillars();
  ...
  function render(tab) {
    ...
    ({ hoje: renderHoje, agenda: renderAgenda, journal: renderJournal, habits: renderHabits, pillars: renderPillars, thesis: renderThesis, review: renderReview }[tab] || renderHoje)(view);
  }
  ...
  /* ----------------------------- PILARES ----------------------------- */
  /* ----------------------------- TESE ----------------------------- */
  // Sítio dedicado ao plano da tese: contagem decrescente até à apresentação (visão de longo
  // prazo), marcos com data alvo, e passos do dia-a-dia (visão micro) — para tornar visível
  // como o trabalho de hoje avança o prazo, e ajudar a arrancar quando falta motivação.
  ...
  function renderThesis(view) {
    const los = Store.get(NS);
    const th = los.thesis || { targetDate: "", milestones: [], tasks: [] };
    $("#subtitle").textContent = "Plano da tese de mestrado";
    ...
    fDate.input.addEventListener("change", () => { Store.update(NS, (s) => { s.thesis.targetDate = fDate.input.value; }, { silent: true }); render("thesis"); });
  ...
  function addMilestone() {
    ...
    const sh = sheet("Novo marco", [f, fd, el("button", { class: "btn btn-primary btn-block", text: "Adicionar", onclick: guardClick(() => {
      ...
      Store.update(NS, (s) => { s.thesis.milestones.push({ id: uid(), text, due: fd.input.value || "", done: false }); });
      sh.close();
    })})]);
    setTimeout(() => f.input.focus(), 50);
  }
  function toggleMilestone(id) { Store.update(NS, (s) => { const m = s.thesis.milestones.find((x) => x.id === id); if (m) m.done = !m.done; }); }
  function removeMilestone(id) { Store.update(NS, (s) => { s.thesis.milestones = s.thesis.milestones.filter((x) => x.id !== id); }); }
  function addThesisTask() {
    ...
    const sh = sheet("Novo passo", [f, fd, el("button", { class: "btn btn-primary btn-block", text: "Adicionar", onclick: guardClick(() => {
      const text = f.input.value.trim(); if (!text) return;
      Store.update(NS, (s) => { s.thesis.tasks.push({ id: uid(), text, due: fd.input.value || "", done: false, createdAt: Date.now(), doneAt: null }); });
      sh.close();
    })})]);
    setTimeout(() => f.input.focus(), 50);
  }
  function toggleThesisTask(id) { Store.update(NS, (s) => { const t = s.thesis.tasks.find((x) => x.id === id); if (t) { t.done = !t.done; t.doneAt = t.done ? Date.now() : null; } }); }
  function removeThesisTask(id) { Store.update(NS, (s) => { s.thesis.tasks = s.thesis.tasks.filter((x) => x.id !== id); }); }
